...
const Month = ({modal, setModal, param, setParam}) => {


  const years = Array.from({length: new Date().getFullYear() - 1999}, (_, i) => i + 2000);
  const month = Array.from({length: 12}, (_, i) => i + 1);
  const [range, setRange] = useState({
    title: new Date().getFullYear(),
    firstName: new Date().getMonth()
  });
  const optionGroups = {
    title: years.map((i) => ({ value: i, label: i })),
    firstName: month.map((i) => ({value: i, label: i}))
  };
